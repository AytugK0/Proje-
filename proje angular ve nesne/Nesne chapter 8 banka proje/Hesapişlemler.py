...
class Bank:
    ...
    def display_bank_statistics():
        total_balance = sum(account.get_balance() for account in Account.accounts)
        total_accounts = len(Account.accounts)
        print(f"Banka'da toplam {total_accounts} hesap bulunmaktadır.")
        print(f"Toplam bakiye: {total_balance} TL")
